...
class SettingsPage(QWidget):
    ...
    def init_ui(self):
        ...
        card_browser = SettingsCard(
            "🌐 Browser Integration",
            "The retired browser bridge is not included in this build.",
        )
        browser_status = QLabel(
            "No browser native host is registered from Settings. "
            "The secure app.barq.browser integration will appear here after "
            "its signed installer and store packages are available."
        )
        browser_status.setWordWrap(True)
        browser_status.setStyleSheet("font-size: 12px; color: #90A4AE;")
        card_browser.card_body.addWidget(browser_status)
